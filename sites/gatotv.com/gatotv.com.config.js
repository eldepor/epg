...
module.exports = {
	...
	days: 2,
	url({ channel, date }) {
	  // Usamos format en lugar de toISO para el log
	...
	parser({ content, date }) {
	  // Usamos format en lugar de toISO para el log
	  console.log(`[DEBUG] Fecha inicial en parser: ${date.format('YYYY-MM-DD')}`)
	  
	  let programs = []
	  const items = parseItems(content)
	  
	  // Ya NO restamos un día a la fecha
	  ...
	  items.forEach((item, i) => {
		...
		console.log(`[DEBUG] Item ${i} - Tiempo original: ${$item('td:nth-child(1) > div > time').attr('datetime')}`)
		
		if (i === 0 && start.hour >= 5) {
		  start = start.plus({ days: 1 })
		  date = date.add(1, 'd')
		  console.log(`[DEBUG] Ajustando fecha para item 0 con hora >= 5: Nueva fecha: ${date.format('YYYY-MM-DD')}`)
		}
		
		let stop = parseStop($item, date)
		
		// Log para ver los tiempos de fin extraídos
		console.log(`[DEBUG] Item ${i} - Tiempo fin original: ${$item('td:nth-child(2) > div > time').attr('datetime')}`)
		
		if (stop < start) {
		  stop = stop.plus({ days: 1 })
		  date = date.add(1, 'd')
		  console.log(`[DEBUG] Ajustando fecha para stop < start: Nueva fecha: ${date.format('YYYY-MM-DD')}`)
		}
  ...
  function parseStart($item, date) {
	...
	// Usamos toString para ser más seguros en lugar de toISO
	console.log(`[DEBUG] parseStart - Resultado UTC: ${dateTime.toString()}`)
  ...
  function parseStop($item, date) {
	...
	// Usamos toString para ser más seguros en lugar de toISO
	console.log(`[DEBUG] parseStop - Resultado UTC: ${dateTime.toString()}`)
